...
interface WindowProps {
  ...
  hideHeader?: boolean; // Новый параметр для скрытия заголовка
}
...
const Window: React.FC<WindowProps> = ({
  ...
  hideHeader = false, // По умолчанию заголовок показывается
}) => {
  ...
  return (
    <motion.div
      ...
      className={`rounded-lg shadow-lg overflow-hidden border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 ${className}`}
      tabIndex={0}
    >
      {/* Статус-бар (мобильный стиль) - показываем только если заголовок не скрыт */}
      {!hideHeader && (
      ...
      {/* Заголовок окна и панель - показываем только если заголовок не скрыт */}
      {!hideHeader && (
        <div
          className="p-2 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center"
        >
      ...
      <div
        ...
        style={{
          maxHeight: hideHeader ? '100%' : 'calc(100% - 70px)',
